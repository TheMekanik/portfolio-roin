...
const Projects = ({language}) => {
  return (
    <div className="border-b border-neutral-900 pb-4">
        ...
        className='my-20 text-center text-4xl'>{language === 'en' ? 'Projects' : 'Proyek'}</motion.h2>
        <div>{PROJECTS[language].map((project, index) => (
            <div key={index} className='mb-8 flex flex-wrap lg:justify-center'>
                ...
                className="w-full lg:w-1/4">
                    <a href={project.link} target='_blank' className="relative group inline-block">
                        ...
                        {language === 'en' ? 'Tap To See More' : 'Selengkapnya'}
                        </p>
                    </a>
                </motion.div>
                ...
                className="w-full max-w-xl lg:w-3/4">
                    <p className='mb-2 font-semibold hover:underline'>{project.title}</p>
